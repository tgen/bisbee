...
pept_file=name + ".all.peptides.csv"
if os.path.exists(pept_file):
 os.remove(pept_file)
top_file=name + ".top.peptides.csv"
...
seq_table=pd.read_csv(name + ".seqTable.csv")
...
peptTable.loc[peptTable.wtIsoform=="iso1","iso1_id"]="en|" + enst[peptTable.wtIsoform=="iso1"]
peptTable.loc[peptTable.wtIsoform=="iso2","iso2_id"]="en|" + enst[peptTable.wtIsoform=="iso2"]
peptTable.loc[peptTable.wtIsoform=="iso1","iso2_id"]=iso1_id[peptTable.wtIsoform=="iso1"]
peptTable.loc[peptTable.wtIsoform=="iso2","iso1_id"]=iso2_id[peptTable.wtIsoform=="iso2"]
...
peptTable.drop(["Unnamed: 0_x","Unnamed: 0_y","seq_id"],inplace=True,axis=1)
...
peptTable.drop(["Unnamed: 0","seq_id","novelSeq_x","novelSeq_y"],inplace=True,axis=1)

peptTable.sort_values(by=["event_id","novelPept","novelSeq","novelSeqLen","wtSeqLen"],ascending=[True,False,True,False,False],inplace=True)
